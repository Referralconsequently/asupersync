...
| Service worker / shared worker | No direct-runtime package surface; bounded broker/coordinator helpers exist | Keep direct `BrowserRuntime` creation out of these hosts. The only shipped package-level affordances are `detectBrowserServiceWorkerBrokerSupport()` / `BrowserServiceWorkerBrokerStore` for bounded service-worker broker registration and `detectBrowserSharedWorkerCoordinatorSupport()` / `createBrowserSharedWorkerCoordinatorSelection()` for bounded shared-worker attach/downgrade flows. |
