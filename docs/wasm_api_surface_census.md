...
## Live Browser Boundary Snapshot (2026-03-15 / `asupersync-1tte9`)

This supplement classifies the live tree by runtime context and capability
family. It is intentionally stricter than roadmap prose: when diagnostics,
docs, and host implementation disagree, the more restrictive host behavior
wins.
...
| Service worker / shared worker | Direct runtime remains fail-closed; bounded package-level broker/coordinator support exists | `packages/browser/src/index.ts` still denies direct `BrowserRuntime` creation on service/shared-worker hosts, while it now exposes `detectBrowserServiceWorkerBrokerSupport()` / `BrowserServiceWorkerBrokerStore` plus `detectBrowserSharedWorkerCoordinatorSupport()` / `createBrowserSharedWorkerCoordinatorSelection()` for bounded registration and attach flows | Keep direct runtime creation out of these hosts themselves; use the bounded service-worker broker or shared-worker coordinator helpers only from their documented entrypoints, and keep downgrade/fallback behavior explicit until those host contracts are promoted deliberately. |
...
1. Dedicated-worker direct runtime is now a shipped browser SDK lane, but it
   ...
   bootstrap + fetch hosting, while service/shared-worker direct runtime
