...
| Rust-authored browser path | Preview-only public builder lane | `tests/wasm_rust_browser_example_contract.rs`, `PATH=/usr/bin:$PATH bash scripts/validate_rust_browser_consumer.sh`, `docs/wasm_quickstart_migration.md` | explicit CI lane `wasm-browser-vnext-surfaces` plus release-lane review in `docs/wasm_ga_readiness_review_board_checklist.md` | Do not promote beyond preview-only while the dispatcher-backed public builder remains narrower than the shipped JS/TS Browser Edition packages |
