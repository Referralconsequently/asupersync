...
| Direct-runtime feasible but not yet shipped | Real substrate exists, but there is no promoted public Browser Edition API/contract yet | service/shared worker direct runtime, public Rust-authored browser bootstrap, public messaging-surface APIs | do not present it as supported; keep it on explicit app-boundary or repo-internal validation lanes until promotion closes | `docs/WASM.md`, `docs/wasm_shared_worker_tenancy_lifecycle_contract.md` |
...
| Browser service worker | deferred and currently unsupported | no | none yet | `@asupersync/browser` reports `reason = "missing_browser_dom"` because the current guard accepts only a DOM window or dedicated worker global | keep this lane on explicit message/data boundaries until its bounded broker contract is validated and promoted |
| Browser shared worker | deferred and currently unsupported | no | none yet | Rust-side execution-ladder diagnostics pin `shared_worker_direct_runtime_not_shipped`; the JS/TS package still rejects the host because only DOM or dedicated worker globals are admitted | follow `docs/wasm_shared_worker_tenancy_lifecycle_contract.md` and keep this lane on explicit message/data boundaries until the tenancy/lifecycle contract is implemented and promoted |
